...
class Env:
    def __init__(
        ...
        self.in_lanes, self.out_lanes, self.jpl, self.jpl_label = (
            self.eng.colight_lanes_collect(self.jids)
        )
        ...
        def phase_lane_numpy(phase_lanes):
            max_inflow_lane_num = max_outflow_lane_num = max(
                max([max([len(j[1]) for j in i]) for i in phase_lanes]),
                max([max([len(j[0]) for j in i]) for i in phase_lanes]),
            )
            phase_lanes_inflow = np.zeros(
                (len(phase_lanes), self.max_action_size, max_inflow_lane_num), dtype=int
            )
            phase_lanes_outflow = np.zeros(
                (len(phase_lanes), self.max_action_size, max_outflow_lane_num),
                dtype=int,
            )
            zero_lanes_inflow = np.zeros(
                (len(phase_lanes), self.max_action_size, max_inflow_lane_num), dtype=int
            )
            zero_lanes_outflow = np.zeros(
                (len(phase_lanes), self.max_action_size, max_outflow_lane_num),
                dtype=int,
            )
            non_zeros_inflow = np.zeros(
                (len(phase_lanes), self.max_action_size, max_inflow_lane_num), dtype=int
            )
            non_zeros_outflow = np.zeros(
                (len(phase_lanes), self.max_action_size, max_outflow_lane_num),
                dtype=int,
            )
            missing_lanes_inflow = np.zeros(
                (len(phase_lanes), self.max_action_size, max_inflow_lane_num), dtype=int
            )
            missing_lanes_outflow = np.zeros(
                (len(phase_lanes), self.max_action_size, max_outflow_lane_num),
                dtype=int,
            )
            missing_phase = np.zeros(
                (len(phase_lanes), self.max_action_size), dtype=int
            )
            ...
            return (
                ...
                zero_lanes_inflow,
                zero_lanes_outflow,
                missing_lanes_inflow,
                missing_lanes_outflow,
                non_zeros_inflow,
                non_zeros_outflow,
                missing_phase,
            )
        ...
        (
            self.phase_lanes_inflow,
            self.phase_lanes_outflow,
            self.zero_lanes_inflow,
            self.zero_lanes_outflow,
            self.missing_lanes_inflow,
            self.missing_lanes_outflow,
            self.non_zeros_inflow,
            self.non_zeros_outflow,
            self.missing_phases,
        ) = phase_lane_numpy(self.phase_lanes)
    ...
    def observe(self):
        ...
        in_cnt_states, out_cnt_states = (
            cnt[self.phase_lanes_inflow],
            cnt[self.phase_lanes_outflow],
        )
        ...
        n = div(  # 相位车道平均等待车辆数
            np.sum(in_cnt_states, axis=2), np.sum(self.non_zeros_inflow, axis=2), -1
        )
...
def main():
    ...
    if args.exp is None:
        path = time.strftime("log/mplight/%Y%m%d-%H%M%S")
    else:
        path = time.strftime(f"log/mplight/{args.exp}/%Y%m%d-%H%M%S")
    ...
    with tqdm(range(args.training_step), ncols=100, smoothing=0.1) as bar:
        for step in bar:
            ...
            if step >= args.training_start and step % args.training_freq == 0:
                ...
                update_times = basic_update_times

...
class Env:
    ...
    def observe(self):
        ...
        # Efficient pressure
        in_cnt_states_A, out_cnt_states_A = (
            cnt[self.phase_lanes_inflow_A],
            cnt[self.phase_lanes_outflow_A],
        )
        ...
        in_pressure_A = div(
            np.sum(in_cnt_states_A, axis=2), np.sum(self.non_zeros_inflow_A, axis=2)
        )
        out_pressure_A = div(
            np.sum(out_cnt_states_A, axis=2), np.sum(self.non_zeros_outflow_A, axis=2)
        )
        pressure_A = in_pressure_A - out_pressure_A
        pressure_A[self.missing_phase == 1] = -1e3

        in_cnt_states_B, out_cnt_states_B = (
            cnt[self.phase_lanes_inflow_B],
            cnt[self.phase_lanes_outflow_B],
        )
        in_cnt_states_B[self.zero_lanes_inflow_B == 1] = 0
        out_cnt_states_B[self.zero_lanes_outflow_B == 1] = 0

        in_pressure_B = div(
            np.sum(in_cnt_states_B, axis=2), np.sum(self.non_zeros_inflow_B, axis=2)
        )
        out_pressure_B = div(
            np.sum(out_cnt_states_B, axis=2), np.sum(self.non_zeros_outflow_B, axis=2)
        )
        ...
        return np.concatenate(
            [pressure_A, running_A, pressure_B, running_B, self.junction_phase_sizes],
            axis=1,
        )
...
class Replay:
    ...
    def sample(self, batchsize, transpose=False):
        s, a, r, sp, d, ac, ns, nm, nns, nnm = zip(
            *random.sample(self._data, min(len(self._data), batchsize))
        )
        if transpose:
            s, a, r, sp, ac, ns, nm, nns, nnm = (
                list(zip(*i)) for i in [s, a, r, sp, ac, ns, nm, nns, nnm]
            )
...
class Actor(nn.Module):
    def __init__(self, obs_size, action_size, mlp_layer):
        super().__init__()
        self.proj = make_mlp(
            obs_size, *(int(i) for i in mlp_layer.split(",")), action_size
        )
        ...
        for step in bar:
            ...
            if step <= args.training_start:
                ...
                with torch.no_grad():
                    m = Q(
                        ...
                        torch.tensor(neighbor_obs, dtype=torch.float32, device=device),
                        torch.tensor(neighbor_mask, dtype=torch.float32, device=device),
                    )
                    m[
                        torch.tensor(
            ...
            if done:
                ...
                if episode_reward / episode_count > best_episode_reward:
                    ...
                    writer.add_scalar(
                        "metric/Best_EpisodeReward", episode_reward / episode_count
                    )
            ...
            if step >= args.training_start and step % args.training_freq == 0:
                ...
                for _ in range(update_times):
                    s, a, r, sp, d, ac, ns, nm, nns, nnm = replay.sample(
                    ...
                    for i in range(int(np.ceil(args.batchsize / mini_batch_size))):
                        s_tmp, a_tmp, r_tmp, sp_tmp, ac_tmp = (
                            ...
                            a[i * mini_batch_size : (i + 1) * mini_batch_size],
                            r[i * mini_batch_size : (i + 1) * mini_batch_size],
                            np.array(
                                sp[i * mini_batch_size : (i + 1) * mini_batch_size]
                            ),
                            np.array(
                                ac[i * mini_batch_size : (i + 1) * mini_batch_size]
                            ),
                        )
                        d_tmp = d.repeat(s_tmp.shape[0], 1).reshape(-1, 1).squeeze(1)
                        s_tmp = (
                            torch.tensor(s_tmp, dtype=torch.float32, device=device)
                            .reshape(-1, s_tmp.shape[-1])
                            .squeeze(1)
                        )
                        a_tmp = (
                            torch.tensor(a_tmp, dtype=torch.long, device=device)
                            .reshape(-1, 1)
                            .squeeze(1)
                        )
                        r_tmp = (
                            torch.tensor(r_tmp, dtype=torch.float32, device=device)
                            .reshape(-1, 1)
                            .squeeze(1)
                        )
                        sp_tmp = (
                            torch.tensor(sp_tmp, dtype=torch.float32, device=device)
                            .reshape(-1, sp_tmp.shape[-1])
                            .squeeze(1)
                        )
                        ac_tmp = (
                            torch.tensor(ac_tmp, dtype=torch.float32, device=device)
                            .reshape(-1, ac_tmp.shape[-1])
                            .squeeze(1)
                        )
                        with torch.no_grad():
                            m = Q_target(sp_tmp)
                            m[ac_tmp == 0] = -1e9
                            y_target = r_tmp + args.gamma * m.max(1).values * (
                                1 - d_tmp
                            )

...
class Env:
    def __init__(
        self, data_path, start_step, step_size, step_count, log_dir, reward, alpha=0
    ):
        ...
        self.jids = [
            i for i, j in enumerate(self.eng.get_junction_phase_counts()) if j > 1
        ]
        ...
        (
            ...
            self.jpl_A,
            self.jpl_B,
        ) = self.eng.advanced_mplight_frap_lanes_collect(js)
        ...
        def phase_lane_numpy(phase_lanes_A, phase_lanes_B):
            max_inflow_lane_num_A = max_outflow_lane_num_A = max(
                max([max([len(j[1]) for j in i]) for i in phase_lanes_A]),
                max([max([len(j[0]) for j in i]) for i in phase_lanes_A]),
            )
            max_inflow_lane_num_B = max_outflow_lane_num_B = max(
                max([max([len(j[1]) for j in i]) for i in phase_lanes_B]),
                max([max([len(j[0]) for j in i]) for i in phase_lanes_B]),
            )
            phase_lanes_inflow_A = np.zeros(
                (len(phase_lanes_A), self.max_action_size, max_inflow_lane_num_A),
                dtype=int,
            )
            phase_lanes_outflow_A = np.zeros(
                (len(phase_lanes_A), self.max_action_size, max_outflow_lane_num_A),
                dtype=int,
            )
            zero_lanes_inflow_A = np.zeros(
                (len(phase_lanes_A), self.max_action_size, max_inflow_lane_num_A),
                dtype=int,
            )
            zero_lanes_outflow_A = np.zeros(
                (len(phase_lanes_A), self.max_action_size, max_outflow_lane_num_A),
                dtype=int,
            )
            non_zeros_inflow_A = np.zeros(
                (len(phase_lanes_A), self.max_action_size, max_inflow_lane_num_A),
                dtype=int,
            )
            non_zeros_outflow_A = np.zeros(
                (len(phase_lanes_A), self.max_action_size, max_outflow_lane_num_A),
                dtype=int,
            )
            missing_lanes_inflow_A = np.zeros(
                (len(phase_lanes_A), self.max_action_size, max_inflow_lane_num_A),
                dtype=int,
            )
            missing_lanes_outflow_A = np.zeros(
                (len(phase_lanes_A), self.max_action_size, max_outflow_lane_num_A),
                dtype=int,
            )

            phase_lanes_inflow_B = np.zeros(
                (len(phase_lanes_B), self.max_action_size, max_inflow_lane_num_B),
                dtype=int,
            )
            phase_lanes_outflow_B = np.zeros(
                (len(phase_lanes_B), self.max_action_size, max_outflow_lane_num_B),
                dtype=int,
            )
            zero_lanes_inflow_B = np.zeros(
                (len(phase_lanes_B), self.max_action_size, max_inflow_lane_num_B),
                dtype=int,
            )
            zero_lanes_outflow_B = np.zeros(
                (len(phase_lanes_B), self.max_action_size, max_outflow_lane_num_B),
                dtype=int,
            )
            non_zeros_inflow_B = np.zeros(
                (len(phase_lanes_B), self.max_action_size, max_inflow_lane_num_B),
                dtype=int,
            )
            non_zeros_outflow_B = np.zeros(
                (len(phase_lanes_B), self.max_action_size, max_outflow_lane_num_B),
                dtype=int,
            )
            missing_lanes_inflow_B = np.zeros(
                (len(phase_lanes_B), self.max_action_size, max_inflow_lane_num_B),
                dtype=int,
            )
            missing_lanes_outflow_B = np.zeros(
                (len(phase_lanes_B), self.max_action_size, max_outflow_lane_num_B),
                dtype=int,
            )

            missing_phase = np.zeros(
                (len(phase_lanes_A), self.max_action_size), dtype=int
            )
            ...
            return (
                phase_lanes_inflow_A,
                phase_lanes_outflow_A,
                zero_lanes_inflow_A,
                zero_lanes_outflow_A,
                non_zeros_inflow_A,
                non_zeros_outflow_A,
                missing_lanes_inflow_A,
                missing_lanes_outflow_A,
                phase_lanes_inflow_B,
                phase_lanes_outflow_B,
                zero_lanes_inflow_B,
                zero_lanes_outflow_B,
                non_zeros_inflow_B,
                non_zeros_outflow_B,
                missing_lanes_inflow_B,
                missing_lanes_outflow_B,
                missing_phase,
            )
        ...
        self.available_actions = np.zeros(
            (len(self.action_sizes), max(self.action_sizes))
        )
        ...
        self.phase_lane_length = [
            [
                [M.lanes[lanes[0][0]].length, M.lanes[lanes[1][0]].length]
                for lanes in phase_lanes
            ]
            for phase_lanes in self.phase_lanes
        ]
        self.junction_type_list = [
            np.array([(m == 3) | (m == 6), (m == 4) | (m == 8)]).astype(int)
            for m in self.action_sizes
        ]
        self.junction_scale_list = np.array(
            [[len(i) / j] for i, j in zip(self.in_lanes, self.action_sizes)]
        )

        (
            self.phase_lanes_inflow_A,
            self.phase_lanes_outflow_A,
            self.zero_lanes_inflow_A,
            self.zero_lanes_outflow_A,
            self.non_zeros_inflow_A,
            self.non_zeros_outflow_A,
            self.missing_lanes_inflow_A,
            self.missing_lanes_outflow_A,
            self.phase_lanes_inflow_B,
            self.phase_lanes_outflow_B,
            self.zero_lanes_inflow_B,
            self.zero_lanes_outflow_B,
            self.non_zeros_inflow_B,
            self.non_zeros_outflow_B,
            self.missing_lanes_inflow_B,
            self.missing_lanes_outflow_B,
            self.missing_phase,
        ) = phase_lane_numpy(self.phase_lanes_A, self.phase_lanes_B)
        self.in_lane_array, self.zero_in_lane_array = in_lane_numpy(self.in_lanes)

        self.junction_phase_sizes = np.zeros(
            (len(self.jids), max(self.action_sizes) + 1)
        )
        ...
        for src_idx, out_lanes in tqdm(enumerate(self.out_lanes)):
            for out_lane in out_lanes:
                ids = [
                    jid
                    for (jid, in_lanes) in enumerate(self.in_lanes)
                    if out_lane in in_lanes
                ]
                if len(ids) != 0:
                    if [src_idx, ids[0]] not in self.junction_graph_edges:
                        self.junction_graph_edges.append([src_idx, ids[0]])
                        self.junction_graph_edge_feats.append(
                            [M.lanes[out_lane].length, 1]
                        )
                        self.junction_src_list[ids[0]].append(src_idx)
                    else:
                        idx = [
                            i
                            for i, value in enumerate(self.junction_graph_edges)
                            if value == [src_idx, ids[0]]
                        ][0]
        ...
        # 标注邻居间的关系
        self.phase_relation = np.zeros(
            (len(self.jids), self.max_neighbor_num, 2), dtype=int
        )  # one-hot type information
        self.neighbor_type = np.zeros(
            (len(self.jids), self.max_neighbor_num), dtype=int
        )
            ...
            for idx, src_idx in enumerate(src_idxs):
                ...
                for dst_phase_idx, dst_phase in enumerate(dst_phase_lanes):
                    ...
                    if len(src_phase_lanes) > 2:
                        if (
                            set(src_phase_lanes[2][1]) & set(dst_in_lanes) != set()
                            or set(src_phase_lanes[1][1]) & set(dst_in_lanes) != set()
                        ):
        ...
        for i, idxs in enumerate(self.junction_src_list):
            if len(idxs) == 0:
                continue
            self.edge_distance[i, : len(idxs), :] = np.array(
                [
                    self.junction_graph_edge_feats[
                        self.junction_graph_edges.index([j, i])
                    ][0]
                    for j in idxs
                ]
            ).reshape(-1, 1)
            self.edge_strength[i, : len(idxs), :] = np.array(
                [
                    self.junction_graph_edge_feats[
                        self.junction_graph_edges.index([j, i])
                    ][1]
        ...
        self.edge_distance = np.concatenate(
            [self.edge_distance, self.edge_strength], axis=2
        )
    ...
    def step(self, action):
        ...
        if self.reward == "sum_queue":
            r = [
                -np.mean(
                    [np.sum(cnt[phase_lanes[0]]) for phase_lanes in self.phase_lanes[i]]
                )
        ...
        if self.reward == "pressure":
            r = [
                -np.abs(np.sum(cnt[self.in_lanes[i]]) - np.sum(cnt[self.out_lanes[i]]))
        ...
        if self.reward == "one_hop_pressure":
            r = np.array(
                [
                    -np.abs(
                        np.sum(cnt[self.in_lanes[i]]) - np.sum(cnt[self.out_lanes[i]])
                    )
                    for i in range(len(self.in_lanes))
                ]
            )
        ...
        return sa, sb, r, done, self.info  # type:ignore
...
class Replay:
    ...
    def sample(self, batchsize, transpose=False):
        sa, sb, a, aoh, r, nsa, nsb, naoh, d = zip(
            *random.sample(self._data, min(len(self._data), batchsize))
        )
        if transpose:
            sa, sb, a, aoh, r, nsa, nsb, naoh = (
                list(zip(*i)) for i in [sa, sb, a, aoh, r, nsa, nsb, naoh]
            )
...
class Actor(nn.Module):
    def __init__(self, obs_size, action_size, device):
        ...
        self.conv2 = nn.Conv2d(
            in_channels=64, out_channels=16, kernel_size=1, stride=1, padding=0
        )
    ...
    def forward(self, obs_h1_A, obs_h1_B, obs_h2):
        phase_representations = torch.zeros(
            (obs_h1_A.shape[0], self.action_sizes, 16),
            dtype=torch.float32,
            device=self.device,
        )
        for phase_idx in range(self.action_sizes):
            ...
            obs_A = self.linear(torch.cat([obs_1_A, obs_2_A], axis=1))  # type:ignore
            obs_1_B = self.relu_h1(self.linear_h1(obs_h1_B[:, [phase_idx]]))
            obs_2_B = self.relu_h1(self.linear_h2(obs_h2[:, [phase_idx]]))
            obs_B = self.linear(torch.cat([obs_1_B, obs_2_B], axis=1))  # type:ignore
            phase_representations[:, phase_idx, :] = obs_A + obs_B

        phase_demand_embedding_matrix = torch.zeros(
            (obs_h1_A.shape[0], 32, self.action_sizes, self.action_sizes - 1),
            dtype=torch.float32,
            device=self.device,
        )
        for phase_idx in range(self.action_sizes):
            ...
            for competing_phase in range(self.action_sizes):
                if phase_idx == competing_phase:
                    continue
                phase_demand_embedding_matrix[:, :, phase_idx, count] = torch.cat(
                    [
                        phase_representations[:, phase_idx],
                        phase_representations[:, competing_phase],
                    ],
                    dim=1,
                )
                count += 1
        phase_demand_embedding_matrix = self.bn1(
            self.conv1(phase_demand_embedding_matrix)
        )
        phase_demand_embedding_matrix = self.bn2(
            self.conv2(phase_demand_embedding_matrix)
        )
        phase_demand_embedding_matrix = self.conv3(
            phase_demand_embedding_matrix
        ).squeeze(1)
        phase_scores = torch.sum(phase_demand_embedding_matrix, axis=2)  # type:ignore
        return phase_scores
...
def main():
    ...
    parser.add_argument(
        "--alpha", type=float, default=0.2, help="balance of neighbour rewards"
    )
    ...
    with tqdm(range(args.training_step), ncols=100, smoothing=0.1) as bar:
        for step in bar:
            ...
            action = []
            for q, oa, ob, oac, a in zip(
                Q, obs_a, obs_b, action_one_hot, env.action_sizes
            ):
                if step < args.training_start or random.random() < eps:  # explore
                    ...
                    action.append(
                        torch.argmax(
                            q(
                                torch.tensor(
                                    oa, dtype=torch.float32, device=device
                                ).unsqueeze(0),
                                torch.tensor(
                                    ob, dtype=torch.float32, device=device
                                ).unsqueeze(0),
                                torch.tensor(
                                    oac, dtype=torch.float32, device=device
                                ).unsqueeze(0),
                            ).squeeze(0)
                        )
                        .cpu()
            ...
            if done:
                writer.add_scalar(
                    "metric/EpisodeReward",
                    episode_reward / (args.steps // args.interval),
                    episode_step,
                )
                if episode_reward > best_episode_reward:
                    best_episode_reward = episode_reward
                    writer.add_scalar(
                        "metric/Best_EpisodeReward",
                        episode_reward / (args.steps // args.interval),
                    )
            ...
            if step >= args.training_start and step % args.training_freq == 0:
                ...
                for _ in range(update_times):
                    sa, sb, a, aoh, r, spa, spb, saoh, d = replay.sample(
                        batch_size, transpose=True
                    )
                    d = torch.tensor(d, dtype=torch.float32, device=device)
                    loss = 0
                    for q, qt, sa, sb, a, aoh, r, spa, spb, saoh in zip(
                        Q, Q_target, sa, sb, a, aoh, r, spa, spb, saoh
                    ):
                        sa = torch.tensor(
                            np.array(sa), dtype=torch.float32, device=device
                        )
                        sb = torch.tensor(
                            np.array(sb), dtype=torch.float32, device=device
                        )
                        a = torch.tensor(np.array(a), dtype=torch.long, device=device)
                        aoh = torch.tensor(
                            np.array(aoh), dtype=torch.float32, device=device
                        )
                        r = torch.tensor(
                            np.array(r), dtype=torch.float32, device=device
                        )
                        spa = torch.tensor(
                            np.array(spa), dtype=torch.float32, device=device
                        )
                        spb = torch.tensor(
                            np.array(spb), dtype=torch.float32, device=device
                        )
                        saoh = torch.tensor(
                            np.array(saoh), dtype=torch.float32, device=device
                        )
                        with torch.no_grad():
                            y_target = r + args.gamma * qt(spa, spb, saoh).max(
                                1
                            ).values * (1 - d)

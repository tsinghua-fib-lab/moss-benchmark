...
from run_advanced_mplight import Env
...
class Replay:
    ...
    def sample(self, batchsize, transpose=False):
        s, a, r, sp, d, ns, nm, nns, nnm = zip(
            *random.sample(self._data, min(len(self._data), batchsize))
        )
        if transpose:
            s, a, r, sp, ns, nm, nns, nnm = (
                list(zip(*i)) for i in [s, a, r, sp, ns, nm, nns, nnm]
            )
...
class Actor(nn.Module):
    def __init__(self, obs_size, action_size, mlp="256,256"):
        ...
        self.Q = make_mlp(
            obs_size + hidden_dim, *(int(i) for i in mlp.split(",")), action_size
        )
...
def main():
    ...
    if args.exp is None:
        path = time.strftime("log/advanced_colight/%Y%m%d-%H%M%S")
    else:
        path = time.strftime(f"log/advanced_colight/{args.exp}/%Y%m%d-%H%M%S")
    ...
    def cal_neighbor_obs(obs, neighbor_idxs, max_neighbor_num):
        ...
        neighbor_obs = np.zeros(
            (obs.shape[0], max_neighbor_num, obs.shape[1]), dtype=np.float32
        )
    ...
    neighbor_obs, neighbor_mask = cal_neighbor_obs(
        obs, env.junction_src_list, env.max_neighbor_num
    )
    obs_sizes = [len(i) for i in obs]
    print(f"{len(obs_sizes)} agents:")

    Q = [Actor(o, a, args.mlp).to(device) for o, a in zip(obs_sizes, env.action_sizes)]
    ...
    with tqdm(range(args.training_step), ncols=100, smoothing=0.1) as bar:
        for step in bar:
            ...
            for q, o, a, ns, nm in zip(
                Q, obs, env.action_sizes, neighbor_obs, neighbor_mask
            ):
                ...
                if step < args.training_start or random.random() < eps:  # explore
                    ...
                    ns = [
                        torch.tensor(
                            ns[i], dtype=torch.float32, device=device
                        ).unsqueeze(0)
                        for i in range(len(ns))
                    ]
                    nm = [
                        torch.tensor(
                            nm[i], dtype=torch.float32, device=device
                        ).unsqueeze(0)
            ...
            next_neighbor_obs, next_neighbor_mask = cal_neighbor_obs(
                next_obs, neighbor_idxs, max_neighbor_num
            )
            ...
            if step >= args.training_start and step % args.training_freq == 0:
                ...
                for _ in range(update_times):
                    s, a, r, sp, d, ns, nm, nns, nnm = replay.sample(
                        batch_size, transpose=True
                    )
                    d = torch.tensor(d, dtype=torch.float32, device=device)
                    loss = 0
                    for q, qt, s, a, r, sp, ns, nm, nns, nnm in zip(
                        Q, Q_target, s, a, r, sp, ns, nm, nns, nnm
                    ):
                        ns, nm, nns, nnm = (
                            np.array(ns),
                            np.array(nm),
                        ...
                        s = torch.tensor(
                            np.array(s), dtype=torch.float32, device=device
                        )
                        a = torch.tensor(a, dtype=torch.long, device=device)
                        r = torch.tensor(r, dtype=torch.float32, device=device)
                        sp = torch.tensor(
                            np.array(sp), dtype=torch.float32, device=device
                        )
                        ns = [
                            torch.tensor(ns[:, i], dtype=torch.float32, device=device)
                            for i in range(max_neighbor_num)
                        ]
                        nm = [
                            torch.tensor(nm[:, i], dtype=torch.float32, device=device)
                            for i in range(max_neighbor_num)
                        ]
                        nns = [
                            torch.tensor(nns[:, i], dtype=torch.float32, device=device)
                            for i in range(max_neighbor_num)
                        ]
                        nnm = [
                            torch.tensor(nnm[:, i], dtype=torch.float32, device=device)
                            for i in range(max_neighbor_num)
                        ]
                        with torch.no_grad():
                            y_target = r + args.gamma * qt(sp, nns, nnm).max(
                                1
                            ).values * (1 - d)

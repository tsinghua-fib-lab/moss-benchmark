...
class Env:
    ...
    def step(self, action):
        ...
        r = (
            np.array([-np.mean(cnt[i]) for i in self.road_lanes])
            .reshape(-1, 2)
            .mean(-1)
        )
...
class Model(nn.Module):
    def __init__(self, env: Env, dim_mlp):
        ...
        self.register_buffer(
            "n1", torch.FloatTensor(1 / env.n1).reshape(1, len(env.n1), 2, 1), False
        )
        self.register_buffer(
            "n2", torch.FloatTensor(1 / env.n2).reshape(1, len(env.n2), 2, 1), False
        )

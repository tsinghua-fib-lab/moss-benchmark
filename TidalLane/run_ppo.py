...
class Env:
    def __init__(self, data_path, start_step, step_size, step_count, log_dir, max_veh_cnt=200):
        ...
        # 计算观测车道
        M = eng.get_map()
        ps = json.load(open(f'{data_path}/road_pairs.json'))
        ps = sum(ps, [])
        self.r_ids = np.array(ps[0::3])
        self.rs = np.array(ps[0::3]).reshape(-1, 2)
        self.ls = np.array(ps[1::3]).reshape(-1, 2)
        ns = np.array(ps[2::3]).reshape(-1, 2)
        self.n1 = ns.copy()
        self.n1[:, 1] -= 1
        self.n2 = ns.copy()
        self.n2[:, 0] -= 1

        self.road_lanes = [sorted(l.index for l in M.roads[r].lanes if l.type == LaneType.DRIVING) for r in self.r_ids]
        l_ids = sorted({i for i in self.road_lanes for i in i})
        l_map = {j: i for i, j in enumerate(l_ids)}
        self.l2r = pad([sorted(l_map[i] for i in i) for i in self.road_lanes], -1)
        self.lane_lengths = np.maximum(1, eng.get_lane_lengths())[l_ids]
        self.l_ids = l_ids
        assert all(self.road_lanes)
        self.obs_size = (len(l_ids), 4)
        self.num_envs = len(ns)

        print(f'{len(ns)} tidal road pairs in total')
    ...
    def step(self, action):
        ...
        r = np.array([-np.mean(cnt[i]) for i in self.road_lanes]).reshape(-1, 2).mean(-1)
...
class Model(nn.Module):
    def __init__(self, env: Env, dim_mlp):
        ...
        self.actor = make_mlp(env.obs_size[-1]*2,  *dim_mlp, 1, sigma=0.01)
        self.register_buffer('l2r', torch.LongTensor(env.l2r), False)
        self.register_buffer('n1',  torch.FloatTensor(1/env.n1).reshape(1, len(env.n1), 2, 1), False)
        self.register_buffer('n2',  torch.FloatTensor(1/env.n2).reshape(1, len(env.n2), 2, 1), False)

    def get_value(self, x):
        l_value = pad_tensor(self.critic(x).squeeze(-1))
        r_value = l_value[:, self.l2r].mean(-1).view(x.size(0), -1, 2).mean(-1)
        return r_value

    def get_action_and_value(self, x, action=None, sample=True, action_only=False):
        l_value = pad_tensor(x, dim=-2)
        r_value = l_value[:, self.l2r].sum(-2)
        B, R, _ = r_value.shape
        p = r_value.view(B, R//2, 2, -1)
        p = torch.cat([p*self.n1, p*self.n2], -1)
        pp_value = self.actor(p).squeeze(-1)
        probs = Categorical(logits=pp_value)
        if action is None:
            action = probs.sample() if sample else torch.argmax(pp_value, -1)
        if action_only:
            return action

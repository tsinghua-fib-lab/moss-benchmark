...
async def main():
    ...
    for _ in range(args.epochs):
        ...
        _routing.close()
        # morning peak 6-12
        cmd = f"python exp/utils/dump_att_tp.py --map_path {loop_map_file} --agent_path {loop_agent_file_to_work} --tp_output_path {loop_tp_path} --output_path {loop_att_path} --device_id {args.device_id}".split(
            " "
        )

...
async def main():
    ...
    while initT > finalT:
        ...
        for jj, _ in enumerate(range(LOCAL_SEARCH_TIMES)):
            ...
            _routing.close()
            # morning peak 6-12
            cmd = f"python3 exp/utils/dump_att_tp.py --map_path {loop_map_file} --start_step {6*3600} --person_path {loop_agent_file_to_work} --tp_output_path {loop_tp_path} --output_path {loop_att_path} --device_id {args.device_id}".split(
                " "
            )

...
class Env:
    def __init__(
        ...
        self.road_free_time = np.array(
            [
                map_lanes_dict[r.lane_ids[0]].length
                / map_lanes_dict[r.lane_ids[0]].max_speed
                for r in self.map.roads
            ]
        )
        self.lane2road = [
            self.road_map[l.parent_id] if l.parent_id < JUNCTION_ID_START else -1
    ...
    def _step(self):
        ...
        while self.persons and self.persons[-1][0] <= self.time + 1:
            ...
            if self.vehicle_policy == VehiclePolicy.SHORTEST:
                ...
                ic = [
                    [i, self.road_fuel_cost[r].sum(), self.road_prices[r].sum()]
                    for i, r in irs
                ]
        ...
        if np.any(mask):
            for i, lane, t in zip(
                np.nonzero(mask)[0],
                self.vehicle_lane[mask],
                self.vehicle_enter_time[mask],
            ):
    ...
    def generate_graph(self):
        ...
        for j in self.map.junctions:
            for l_id in j.lane_ids:
                ...
                if (
                    l.type == LaneType.LANE_TYPE_DRIVING
                    and l.predecessors
                    and l.successors
                ):
                    edges.append(
                        (
                            self.road_map[
                                self.map_lanes_dict[l.predecessors[0].id].parent_id
                            ],
                            self.road_map[
                                self.map_lanes_dict[l.successors[0].id].parent_id
                            ],
                        )
                    )
        edges = sorted(set(edges))
        return (node_count, torch.tensor(edges, dtype=torch.long).T.contiguous())
...
class DeltaController:
    ...
    def step(self):
        ...
        dt = np.maximum(0, dt)
        self.env.road_prices[:] = (
            1 - self.R
        ) * self.env.road_prices + self.R * self.beta * dt
        self.env.step()
...
class EGCNController:
    ...
    def step(self):
        ...
        if done:
            ...
            self.obs = next_obs
        _end_time = time.time()
        print(f"sim cost {_end_time-_start_time}")
        if self.args.egcn_train:
            ...
            _end_time = time.time()
            print(f"rl cost {_end_time-_start_time}")
